...
It runs both bots via `asyncio.gather(..., return_exceptions=True)`, so they can't fail each other.
...
1. Type a question
2. Calls `/compare`
3. Displays both answers as "Response 1" and "Response 2". It's randomly swapped on every run, so the evaluator can't tell which bot is which
...
The pledge bot reads `interventions/pledge` (control, unchanged). The pledge2 bot reads `interventions/pledge2` (same guardrails, teen prompt).
